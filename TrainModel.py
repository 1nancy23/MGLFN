...
Loss_te=CrossEntropyLoss(weight=torch.Tensor([1.8,2.2,2.2,2.2,2.2,1.6,0.1,0.1,0.2]).cuda())
from funcs import *
...
def train_epoch(model, train_loader, criterion, edge_loss, sekloss, optimizer, device, scaler,
    ...
    for batch_idx, (images, labels) in enumerate(progress_bar):
        ...
        # rot_degree = np.pi * (random.randint(-25, 25) / 180.0)
        # images = rot_img(images, rot_degree)
        # labelsx = torch.unsqueeze(labels, dim=1).float()
        # labels = torch.squeeze(rot_img(labelsx, rot_degree),dim=1).long()

        # labels_edge=edge_detector(torch.cat((labelsx,labelsx,labelsx),dim=1))
        # labels_edge = rot_img(labels_edge,rot_degree)
        ...
        with autocast():
            ...
            loss_outputs = Loss_te(outputs, labels)
            aux_loss = Loss_te(outputs,labels)
...
def train_model(model, train_loader, val_loader, num_epochs, device, save_dir='checkpoints',
    ...
    # 优化器设置
    optimizer = optim.AdamW(model.parameters(), lr=0.00002, weight_decay=0.12)
...
def test_model(model, test_loader, device, checkpoint_path=None):
    ...
    print("开始测试 (FP16)...")
    with torch.no_grad():
        progress_bar = tqdm(test_loader, desc="Testing (FP16)")

        for images, labels in progress_bar:

            # print(type(paths_images))
            images, labels = images.to(device, non_blocking=True), labels.to(device, non_blocking=True)
            ...
            # 计算Dice和SSIM
            dice_scores, _ = calculate_dice_coefficient(predictions, labels, num_classes=9)
            ssim_scores, _ = calculate_ssim(predictions, labels, num_classes=9)
...
def test_model_hebing(model, test_loader, device, checkpoint_path=None):
    """
    支持半精度的测试函数 (Top-2 容错模式)
    只要真实标签出现在预测概率的前三名中，即视为该像素预测正确。
    """
    if checkpoint_path:
        print(f"加载模型: {checkpoint_path}")
        checkpoint = torch.load(checkpoint_path, map_location=device)
    ...
    print("开始测试 (FP16) [Top-2 容错评估模式]...")
    with torch.no_grad():
        progress_bar = tqdm(test_loader, desc="Testing (Top-2)")
        ...
        for images, labels in progress_bar:
            ...
            # 1. 获取 Top-1 预测 (原始最强预测，用于未命中Top-3时的保底)
            top1_preds = torch.argmax(logits, dim=1)

            # 2. 获取 Top-3 预测的索引 [B, 3, H, W]
            # values, indices = torch.topk(input, k, dim)
            _, top3_indices = torch.topk(logits, k=1, dim=1)

            # 3. 检查标签是否在 Top-3 中
            # labels 维度是 [B, H, W]，需要扩展为 [B, 1, H, W] 以便与 top3_indices 广播对比
            # 结果 match_mask 维度为 [B, 3, H, W]
            labels_expanded = labels.unsqueeze(1)
            match_mask = (top3_indices == labels_expanded)

            # 4. 压缩维度：只要 Top-3 中任意一个位置命中了标签，该像素就是 True
            # is_in_top3 维度为 [B, H, W]
            is_in_top3 = match_mask.any(dim=1)

            # 5. 生成"修正后"的预测图
            # 如果在 Top-3 中，预测值 = 标签值 (视为正确)
            # 如果不在 Top-3 中，预测值 = Top-1 预测值 (保留原始错误)
            refined_preds = torch.where(is_in_top3, labels, top1_preds)

            # ============================================================
            # 指标计算 (使用 refined_preds)
            # ============================================================

            metrics.update(refined_preds, labels)

            dice_scores, _ = calculate_dice_coefficient(refined_preds, labels, num_classes=9)
            ssim_scores, _ = calculate_ssim(refined_preds, labels, num_classes=9)
    ...
    print("\n测试结果 (Top-3 命中即视为正确):")
    print(f"Top-3 像素准确率: {test_metrics['pixel_accuracy']:.4f}")
    print(f"Top-3 平均准确率: {test_metrics['mean_accuracy']:.4f}")
    print(f"Top-3 平均IoU: {test_metrics['mean_iou']:.4f}")
    print(f"Top-3 平均Dice: {overall_dice:.4f}")
    print(f"Top-3 平均SSIM: {overall_ssim:.4f}")
...
def test_model_and_save_low_iou_images(
        model, test_loader, device,
        checkpoint_path=None,
        iou_threshold=0.70,
        save_dir_low_iou='D:/ZJF/种植作物分类/Datasplit/low_iou_samples_2'
):
    """
    计算每张图片的IoU，若iou均值低于iou_threshold，
    复制对应图片、标签，并保存预测结果可视化到指定目录
    """
    import os
    import shutil
    ...
    def label_to_color(label_array):
        ...
        for cls in range(len(COLORMAP)):
            color_img[label_array == cls] = COLORMAP[cls]
        return color_img
    ...
    # 创建保存目录
    os.makedirs(save_dir_low_iou, exist_ok=True)
    images_save_dir = os.path.join(save_dir_low_iou, 'images')
    labels_save_dir = os.path.join(save_dir_low_iou, 'labels')
    preds_save_dir = os.path.join(save_dir_low_iou, 'predictions')  # 预测结果灰度图
    preds_vis_save_dir = os.path.join(save_dir_low_iou, 'predictions_vis')  # 预测结果可视化
    compare_save_dir = os.path.join(save_dir_low_iou, 'comparison')  # 对比图

    os.makedirs(images_save_dir, exist_ok=True)
    os.makedirs(labels_save_dir, exist_ok=True)
    os.makedirs(preds_save_dir, exist_ok=True)
    os.makedirs(preds_vis_save_dir, exist_ok=True)
    os.makedirs(compare_save_dir, exist_ok=True)

    low_iou_count = 0
    total_images = 0

    print("开始测试计算单图IoU并保存低IoU样本...")
    with torch.no_grad():
        progress_bar = tqdm(test_loader, desc="Testing")

        for images, labels, paths_images, paths_labels in progress_bar:
            images, labels = images.to(device, non_blocking=True), labels.to(device, non_blocking=True)
            ...
            for i in range(batch_size):
                pred_i = predictions[i]  # (H, W)
                label_i = labels[i]  # (H, W)
                image_i = images[i]  # (C, H, W)
                ...
                mean_iou_img = sum(iou_scores) / len(iou_scores)

                total_images += 1
                if mean_iou_img < iou_threshold:
                    low_iou_count += 1

                    src_img_path = paths_images[i]
                    src_label_path = paths_labels[i]
                    base_name = os.path.splitext(os.path.basename(src_img_path))[0]

                    # 1. 复制原始图片
                    dst_img_path = os.path.join(images_save_dir, os.path.basename(src_img_path))
                    shutil.copy(src_img_path, dst_img_path)

                    # 2. 复制标签
                    dst_label_path = os.path.join(labels_save_dir, os.path.basename(src_label_path))
                    shutil.copy(src_label_path, dst_label_path)

                    # 3. 保存预测结果（灰度图）
                    pred_gray = Image.fromarray(pred_np.astype(np.uint8), mode='L')
                    pred_gray.save(os.path.join(preds_save_dir, f"{base_name}_pred.png"))

                    # 4. 保存预测结果可视化（彩色）
                    pred_color = label_to_color(pred_np)
                    pred_color_img = Image.fromarray(pred_color)
                    pred_color_img.save(os.path.join(preds_vis_save_dir, f"{base_name}_pred_vis.png"))

                    # 5. 保存对比图（原图、标签、预测 三联图）
                    try:
                        # 原图反归一化（根据你的预处理调整）
                        img_np = image_i.cpu().numpy().transpose(1, 2, 0)
                        # 假设归一化参数
                        ...
                        label_color = label_to_color(label_np)
                        ...
                        axes[2].set_title(f'Prediction (mIoU={mean_iou_img:.3f})')
                        axes[2].axis('off')

                        plt.tight_layout()
                        plt.savefig(os.path.join(compare_save_dir, f"{base_name}_compare.png"),
            ...
            progress_bar.set_postfix({
                'LowIoUCount': low_iou_count,
                'TotalImgs': total_images,
                'Ratio': f"{100 * low_iou_count / (total_images + 1e-8):.2f}%"
            })
    ...
    print(f"\n低IoU样本已保存至: {save_dir_low_iou}")
    print(f"  - 原图: {images_save_dir}")
    print(f"  - 标签: {labels_save_dir}")
    print(f"  - 预测灰度图: {preds_save_dir}")
    print(f"  - 预测可视化: {preds_vis_save_dir}")
    print(f"  - 对比图: {compare_save_dir}")

    return test_metrics
from D2LS.model import DynamicDictionaryLearning
from PSPNet import PSPNet
# import segmentation_models_pytorch as smp
# model = smp.Unet(
#     encoder_name="timm-efficientnet-b0", # 使用 timm 的 efficientnet_b0
#     encoder_weights="imagenet",          # 使用 timm 的预训练权重
#     in_channels=3,
#     classes=10                           # 分割类别数
# )
def main():
    config = {
        'train_image_dir': 'D:/ZJF/种植作物分类/Datasplit/train_enhaced/images',
        'train_label_dir': 'D:/ZJF/种植作物分类/Datasplit/train_enhaced/labels',
        'val_image_dir': 'D:/ZJF/种植作物分类/DataSplit/train/images',
        'val_label_dir': 'D:/ZJF/种植作物分类/Datas/GT',
        'test_image_dir': 'D:/ZJF/种植作物分类/Datasplit/test/Datasplit_split/test/images',
        'test_label_dir': 'D:/ZJF/种植作物分类/Datasplit/test/Datasplit_split/test/labels',
        'batch_size':6,  # 可以适当增加batch size，因为半精度占用内存更少
        'num_epochs': 160,
        'num_workers': 12,
        'train_subset_ratio': 1.0,  # 每个epoch使用40%的训练数据
        'save_dir': 'UNes_1'  # 标识半精度训练和40%数据
    }
...
#     load_pretrained_partial(model,'./UNetFormer_ResMMFormer/best_model.pt')
    ...
    # checkpoint_path = './UNes_1/best_model.pt'
    # checkpoint = torch.load(checkpoint_path, map_location=device)
    # model.load_state_dict(checkpoint['model_state_dict'], strict=False)
    # print("已加载预训练模型")
    ...
    if os.path.exists(best_model_path):
        test_metrics = test_model_hebing(model, test_loader, device, best_model_path)
        print("\n最终测试结果:")
        print(test_metrics)
    else:
        test_metrics = test_model_hebing(model, test_loader, device)
        print("\n最终测试结果:")
        print(test_metrics)
